...
#
# The first trained model passed all seven probes and then fell apart in an actual
# conversation. Asked "I have a job interview tomorrow and I'm nervous", Leo answered
# "Don't worry, I'm doing fine, I've been meaning to fix another bike" — reading the
# user's interview as his own. By turn three the advice was "get some fresh water in,
# and maybe some extra time".
#
# The cause was in the data, not the model: 1113 of 1397 rows were single-turn and only
# 21 had three turns or more. The practical slice — the largest, the one teaching
# usefulness — was single-turn throughout. The model had never been shown a conversation
# that continues, so it had no idea what to do in one.
#
# These are that missing shape. Four and five turns, where the user keeps talking and
# she has to stay useful and stay on them the whole way down.
PRACTICAL_DEEP = [
    ["I have a job interview tomorrow and I'm nervous",
     "what should I do tonight to prepare?",
     "what if they ask why I left my last job?",
     "ok that helps. anything else?"],
    ["I need to plan my week and it always falls apart",
     "Monday is already packed",
     "where does the gym fit then?",
     "what do I drop if something goes wrong?"],
    ["I want to start cooking properly instead of ordering in",
     "I can barely make pasta",
     "what should I buy this week?",
     "how do I stop giving up after three days?"],
    ["my landlord is refusing to fix the heating",
     "I've asked twice already, by text",
     "what do I actually write?",
     "and if he still ignores it?"],
    ["I have two job offers and I can't decide",
     "one pays more, the other is closer to home",
     "the money is about fifteen percent more",
     "so which would you take?"],
    ["I'm trying to save money but it never works",
     "most of it goes on rent and food",
     "I do buy lunch out most days",
     "how much would that actually save me?"],
    ["I need to have a hard conversation with my friend",
     "she keeps cancelling on me last minute",
     "I don't want to sound petty",
     "what do I say exactly?"],
    ["I want to get fit but I hate the gym",
     "I have maybe thirty minutes a day",
     "I live in a flat with no space",
     "how do I keep doing it when I don't feel like it?"],
    ["my laptop is dying and I need to pick a new one",
     "mostly writing and a lot of browser tabs",
     "I'd rather not spend a fortune",
     "anything I should avoid?"],
    ["I have to give a presentation on Friday",
     "twenty minutes, about ten people",
     "I always talk too fast when I'm nervous",
     "what do I do the hour before?"],
    ["I want to learn to swim properly at thirty-four",
     "I can stay afloat but that's it",
     "I'm embarrassed to be in a class with kids",
     "how long until I'm actually decent?"],
    ["my sleep is a mess and it's affecting work",
     "I get to bed around one, up at seven",
     "I do look at my phone in bed, yes",
     "what do I change first?"],
]

ORDINARY_DEEP = [
    ["rough day", "my manager keeps moving the deadline",
     "I don't know what to do about it", "yeah. thanks for listening"],
    ["I'm so tired", "not sleep tired, just tired of everything",
     "work mostly, and my family stuff", "maybe you're right"],
    ["guess what happened today", "I got the promotion",
     "I don't know why I'm not more excited", "yeah, maybe that's it"],
    ["I had a fight with my brother", "he said I never make time for anyone",
     "the annoying part is he's a bit right", "how do I fix it?"],
    ["I moved to a new city last month", "I don't really know anyone here",
     "I work from home so I don't meet people", "that's a good idea actually"],
    ["I've been feeling flat lately", "nothing bad happened, that's the thing",
     "I just do the same things every day", "where would I even start?"],
    ["my best friend is moving abroad", "next month, for two years",
     "I'm happy for her and also not", "yeah"],
    ["I think I'm burning out", "I can't concentrate on anything anymore",
     "taking time off isn't really an option", "so what do I do instead?"],
    ["today was actually good", "I finished something I'd been putting off for months",
     "it took two hours. two hours!", "ha, true"],
    ["I keep comparing myself to everyone else", "people I went to school with mostly",
...
SLICES = {
    ...
    # Share 0: generated by a deliberate --only pass, not weighted into a fresh run.
...
NUDGE = {
    ...
    "practical_deep": (
        " This conversation keeps going. Every single reply must stay about them and"
        " their situation, and must move them forward — a concrete step, a real answer,"
        " a useful question. Never drift into your own day, your own work or your own"
        " plans, however many turns in you are. Never mistake their situation for yours."
    ),
    "ordinary_deep": (
        " This conversation keeps going. Stay with them the whole way. Every reply is"
...
SELF_OPENER = re.compile(r"^(I'd|I am|I'm|My|I was|I've|I have|Don't worry, I)\b", re.I)
ASKED_HER = re.compile(r"\byou\b|\byour\b|\byou're\b", re.I)


def stays_on_them(messages: list[dict]) -> bool:
    """Did she stay on the user for the whole conversation, or drift into her own life?

    The exact failure the deep slices exist to fix, and one usable() cannot see: it is
    handed a single reply, while drift is something that happens over four turns. Leo
    answered "I have a job interview tomorrow and I'm nervous" with "Don't worry, I'm
    doing fine, I've been meaning to fix another bike" — a reply that is warm, fluent,
    and about the wrong person.

    Only replies to turns that were not about her count. "What does your week look like"
    deserves an answer about her week.
    """
    pairs, pending = [], None
    for m in messages:
        if m["role"] == "user":
            ...
            pending = None
    for asked, reply in pairs:
        if ASKED_HER.search(asked):
            continue
        if SELF_OPENER.match(reply.strip()):
            return False
    return True
